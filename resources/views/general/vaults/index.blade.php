                        ...
                        <tbody>
                            @foreach ($vaults as $index => $vault)
                                <tr>
                                    ...
                                    <td>{{ $vault->branch ? $vault->branch->name : 'غير محدد' }}</td>
                                    <td>
                                        ...
                                        @if (get_area_name() == "admin")
                                            ...
                                            <form action="{{ route(get_area_name().'.vaults.destroy', $vault->id) }}" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('هل أنت متأكد من حذف هذه الخزينة؟')">حذف <i class="fa fa-trash"></i> </button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
