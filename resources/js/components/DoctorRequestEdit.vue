<template>
  <div class="">
    ...
    <div v-if="loading" class="text-center py-5">
      ...
      <p class="mt-3 text-muted">جاري تحميل بيانات الطلب...</p>
    </div>
    ...
    <form v-else @submit.prevent="handleSubmit">
      ...
      <div v-show="currentStep === 1" class="step-content">
        <div class="card mb-4">
          <div class="card-header bg-primary text-white">
            <i class="fas fa-user-md me-2"></i>الخطوة الأولى: بيانات الطبيب
          </div>
          <div class="card-body">
            <div v-if="selectedDoctor" class="doctor-info p-3 bg-light rounded">
              <h5><i class="fas fa-user-md me-2"></i>{{ selectedDoctor.name }}</h5>
              <p class="mb-0 text-muted">كود الطبيب: {{ selectedDoctor.code }}</p>
              <small class="text-info">لا يمكن تغيير الطبيب في وضع التعديل</small>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-show="currentStep === 2" class="step-content">
        <div class="card mb-4">
          <div class="card-header bg-primary text-white">
            <i class="fas fa-concierge-bell me-2"></i>الخطوة الثانية: تعديل الخدمات
          </div>
          <div class="card-body">
            <label class="form-label required">اختر خدمة جديدة</label>
            <Select
                  ...
                  <div class="card border-primary">
                    <div class="card-body">
                      <div class="d-flex justify-content-between align-items-start mb-2">
                        <h6 class="card-title mb-0">{{ service.service_name || service.label?.split('(')[0] }}</h6>
                        <button type="button" class="btn btn-sm btn-outline-danger" @click="removeService(index)">
                          <i class="fas fa-times"></i>
                        </button>
                      </div>
                      <div class="text-primary fw-bold mb-3">{{ formatCurrency(service.amount) }} د.ل</div>
                      
                      <!-- خيارات جهة العمل للخدمات المحددة -->
                      <div v-if="[8,22,34].includes(service.id || service.pricing_id)" class="mb-3">
                        ...
                        <div class="form-check">
                          ...
                          />
                          <label class="form-check-label small">مع ذكر جهة العمل</label>
                        </div>
                        <div class="form-check">
                          <input 
                            class="form-check-input" 
                      ...
                      <div class="mb-2">
                        ...
                        <!-- عرض الملف الموجود -->
                        <div v-if="service.file_path && !service.newFile" class="mb-2">
                          <div class="d-flex align-items-center bg-success bg-opacity-10 p-2 rounded">
                            <i class="fas fa-file text-success me-2"></i>
                            <span class="flex-grow-1">{{ service.file_name || 'ملف موجود' }}</span>
                            <a :href="getFileUrl(service.file_path)" target="_blank" class="btn btn-sm btn-outline-success me-2">
                        ...
                        <input 
                          ...
                          :class="{ 
                            'is-invalid': service.file_required && !service.file && !service.file_path,
                            'is-valid': service.file || service.file_path 
                          }"
                          @change="onServiceFileChange($event, index)" 
                          accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
                          :required="service.file_required && !service.file_path"
                        />
                        
                        <!-- عرض اسم الملف الجديد المرفوع -->
                        ...
                        <div v-if="service.file_required && !service.file && !service.file_path" class="invalid-feedback">
      ...
      <div v-show="currentStep === 3" class="step-content">
        <div class="card mb-4">
          <div class="card-header bg-primary text-white">
            <i class="fas fa-envelope me-2"></i>الخطوة الثالثة: تعديل البريد الإلكتروني
            <span class="badge bg-light text-dark ms-2">{{ unitPrice }} د.ل لكل بريد</span>
          </div>
          <div class="card-body">
      ...
      <div v-show="currentStep === 4" class="step-content">
        <div class="card mb-4">
          <div class="card-header bg-primary text-white">
            <i class="fas fa-globe me-2"></i>الخطوة الرابعة: تعديل الدول المستهدفة
          </div>
          <div class="card-body">
      ...
      <div v-show="currentStep === 5" class="step-content">
        ...
        <div class="card mb-4">
          <div class="card-header bg-primary text-white">
            <i class="fas fa-sticky-note me-2"></i>الخطوة الخامسة: معلومات إضافية ومراجعة
          </div>
          <div class="card-body">
            <div class="mb-3">
        ...
        <div class="card mb-4">
          <div class="card-header bg-success text-white">
            <i class="fas fa-clipboard-check me-2"></i>ملخص الطلب المحدث
          </div>
          <div class="card-body">
            <!-- معلومات الطبيب -->
            ...
            <div class="summary-section mb-4" v-if="selectedServices.length">
              ...
              <ul class="list-unstyled mb-0">
                <li v-for="service in selectedServices" :key="service.id" class="d-flex justify-content-between">
                  <span>{{ service.service_name || service.label?.split('(')[0] }}</span>
                  <span class="text-primary fw-bold">{{ formatCurrency(service.amount) }} د.ل</span>
                </li>
              </ul>
            </div>
...
const props = defineProps({ 
  doctorMailId: [Number, String],
  doctorId: [Number, String] 
})

// Steps configuration
const steps = ref([
  { title: 'بيانات الطبيب', icon: 'fas fa-user-md' },
  { title: 'تعديل الخدمات', icon: 'fas fa-concierge-bell' },
  { title: 'تعديل البريد', icon: 'fas fa-envelope' },
  { title: 'تعديل الدول', icon: 'fas fa-globe' },
  { title: 'المراجعة والحفظ', icon: 'fas fa-save' }
])
...
async function loadDoctorMailData() {
  try {
    const { data } = await axios.get(`/api/doctor-mails/${props.doctorMailId}/data`, {
      headers: {
    ...
    // تحميل بيانات الطبيب
    selectedDoctor.value = originalData.value.doctor
    
    // تحميل الخدمات
    selectedServices.value = originalData.value.services.map(service => ({
    ...
    // تحميل الإيميلات
    addedEmails.value = originalData.value.emails
    
    // تحميل الدول
    selectedCountries.value = originalData.value.countries
    
    // تحميل المعلومات الإضافية
    notes.value = originalData.value.notes || ''
    extractedBefore.value = originalData.value.extracted_before || false
    lastExtractYear.value = originalData.value.last_extract_year || ''
    unitPrice.value = originalData.value.totals?.email_unit_price || 50

    // تحميل خدمات الطبيب
    await loadDoctorServices()
    
  } catch (error) {
    console.error('Error loading doctor mail data:', error)
    Swal.fire('خطأ', 'فشل في تحميل بيانات الطلب', 'error')
  }
}

// تحميل خدمات الطبيب
async function loadDoctorServices() {
  if (!selectedDoctor.value?.type) return

  try {
    const { data: mailPricings } = await axios.get('/api/pricings', {
      params: { type: 'mail', doctor_type: selectedDoctor.value.type }
    })
    
    servicesFull.value = mailPricings.map(p => ({
      id: p.id,
      label: `${p.name} (${Number(p.amount || 0).toFixed(2)} د.ل)`,
    ...
    case 2:
      return selectedServices.value.length > 0 && 
             selectedServices.value.every(s => 
               ![8,22,34].includes(s.id || s.pricing_id) || s.work_mention
             ) &&
             selectedServices.value.every(s => 
               !s.file_required || s.file || s.file_path
             )
    case 3:
...
function onServiceFileChange(event, index) {
  ...
  selectedServices.value[index].newFile = true
}

// دالة إزالة الملف الجديد
function removeServiceFile(index) {
  selectedServices.value[index].file = null
  selectedServices.value[index].newFile = false
...
// دالة إزالة الملف الموجود
function removeExistingFile(index) {
  selectedServices.value[index].file_path = null
  selectedServices.value[index].file_name = null
}
...
const hasRequiredFilesNotUploaded = computed(() => {
  return selectedServices.value.some(service => 
    service.file_required && !service.file && !service.file_path
  )
})
...
async function handleSubmit() {
  if (!canSubmit.value) {
    ...
    if (!selectedDoctor.value) {
      errorMessage = 'خطأ في بيانات الطبيب'
    ...
    } else if (selectedServices.value.some(s => [8,22,34].includes(s.id || s.pricing_id) && !s.work_mention)) {
  ...
  try {
    ...
    selectedServices.value.forEach((service, index) => {
      formData.append(`services[${index}][id]`, service.id || service.pricing_id)
      ...
      // الملف الموجود
      if (service.file_path && !service.newFile) {
        formData.append(`services[${index}][existing_file_path]`, service.file_path)
        formData.append(`services[${index}][existing_file_name]`, service.file_name || '')
      }
    ...
    const response = await axios.post(`/doctor/doctor-mails/${props.doctorMailId}/update`, formData, {
      headers: {
    ...
    if (response.data.success) {
      await Swal.fire({
        ...
        text: 'سيتم مراجعة التعديلات وإشعارك بالنتيجة',
        timer: 2000,
        showConfirmButton: false
      })

      // العودة للوحة التحكم
      window.location = '/doctor/doctor-mails?requests=1'
    } else {
      throw new Error(response.data.message || 'فشل في تحديث الطلب')
    }
